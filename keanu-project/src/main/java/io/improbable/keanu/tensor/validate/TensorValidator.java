...
public interface TensorValidator<DATATYPE, TENSOR extends Tensor<DATATYPE>>
  ...
  static <DATATYPE, TENSOR extends Tensor<DATATYPE>> TensorValidator<DATATYPE, TENSOR> thatReplaces(
      DATATYPE oldValue, DATATYPE newValue) {
    return new TensorCheckAndRespondValidator<>(
        new TensorValueNotEqualsCheck<>(oldValue), TensorValidationPolicy.changeValueTo(newValue));
  }
