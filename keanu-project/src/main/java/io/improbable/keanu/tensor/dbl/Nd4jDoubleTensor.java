...
public class Nd4jDoubleTensor implements DoubleTensor {
  ...
  public static Nd4jDoubleTensor linspace(double start, double end, int numberOfPoints) {
    return new Nd4jDoubleTensor(
        TypedINDArrayFactory.linspace(start, end, numberOfPoints, BUFFER_TYPE));
  }
  ...
  public static Nd4jDoubleTensor arange(double start, double end, double stepSize) {
    ...
    INDArray arangeWithStep =
        TypedINDArrayFactory.arange(0, stepCount, BUFFER_TYPE).muli(stepSize).addi(start);
  ...
  public DoubleTensor tensorMultiply(DoubleTensor value, int[] dimsLeft, int[] dimsRight) {
    INDArray tensorMmulResult =
        Nd4j.tensorMmul(tensor, unsafeGetNd4J(value), new int[][] {dimsLeft, dimsRight});
  ...
  /**
   * This is identical to log().times(y), except that it changes NaN results to 0. This is important
   * when calculating 0log0, which should return 0 See
   ...
   * @return A new DoubleTensor instance only if <i>this</i> has a length of 1 and right operand has
   *     a length greater than 1. Otherwise return <i>this</i>.
   */
  @Override
  ...
  public DoubleTensor getGreaterThanOrEqualToMask(DoubleTensor greaterThanOrEqualToThis) {
    ...
    if (greaterThanOrEqualToThis.isScalar()) {
      ...
      Nd4j.getExecutioner()
          .exec(new OldGreaterThanOrEqual(mask, greaterThanThisArray, mask, mask.length()));
    }
  ...
  public DoubleTensor getLessThanMask(DoubleTensor lessThanThis) {
    ...
    if (lessThanThis.isScalar()) {
      ...
      Nd4j.getExecutioner().exec(new OldLessThan(mask, lessThanThisArray, mask, mask.length()));
    }
  ...
  public DoubleTensor getLessThanOrEqualToMask(DoubleTensor lessThanOrEqualToThis) {
    ...
    if (lessThanOrEqualToThis.isScalar()) {
      ...
      Nd4j.getExecutioner()
          .exec(new OldLessThanOrEqual(mask, lessThanOrEqualToThisArray, mask, mask.length()));
    }
  ...
  public DoubleTensor setWithMaskInPlace(DoubleTensor mask, Double value) {
    ...
    Nd4j.getExecutioner().exec(new CompareAndSet(maskDup, value, Conditions.equals(trueValue)));

    Nd4j.getExecutioner()
        .exec(new CompareAndSet(tensor, maskDup, Conditions.notEquals(falseValue)));
   ...
   * @return pieces of the tensor slice in the order specified by splitAtIndices. To get pieces that
   *     encompasses the entire tensor, the last index in the splitAtIndices must be the length of
   *     the dimension being slice on.
   *     <p>e.g A = [ 1, 2, 3, 4, 5, 6 7, 8, 9, 1, 2, 3 ]
   *     <p>A.slice(0, [1]) gives List([1, 2, 3, 4, 5, 6]) A.slice(0, [1, 2]) gives List([1, 2, 3,
   *     4, 5, 6], [7, 8, 9, 1, 2, 3]
   *     <p>A.slice(1, [1, 3, 6]) gives List( [1, [2, 3 , [4, 5, 6, 7] 8, 9] 1, 2, 3] )
   */
  @Override
