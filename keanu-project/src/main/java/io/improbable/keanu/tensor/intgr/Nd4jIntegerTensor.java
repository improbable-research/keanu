...
public class Nd4jIntegerTensor implements IntegerTensor {
      ...
      Nd4j.getExecutioner()
          .exec(
              new OldGreaterThanOrEqual(
                  mask,
                  TypedINDArrayFactory.valueArrayOf(
                      mask.shape(), greaterThanOrEqualToThis.scalar(), BUFFER_TYPE),
                  mask,
                  mask.length()));
    } else {
      ...
      Nd4j.getExecutioner()
          .exec(
              new OldLessThanOrEqual(
                  mask,
                  TypedINDArrayFactory.valueArrayOf(
                      mask.shape(), lessThanOrEqualToThis.scalar(), BUFFER_TYPE),
                  mask,
                  mask.length()));
    } else {
  ...
  public IntegerTensor setWithMaskInPlace(IntegerTensor mask, Integer value) {
    ...
    if (value == 0.0) {
      ...
      Nd4j.getExecutioner().exec(new CompareAndSet(tensor, maskDup, Conditions.notEquals(0.0)));
    }
