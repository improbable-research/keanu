...
public class PartialDerivatives {
    ...
    public static PartialDerivatives matrixMultiplyAlongOfDimensions(
        ...
        for (Map.Entry<VertexId, DoubleTensor> partial :
                partials.derivativeWithRespectTo.entrySet()) {
            ...
            if (partialIsLeft) {
                ...
                result =
                        multiplier.tensorMultiply(partial.getValue(), new int[] {1}, new int[] {0});
            }
        ...
        for (Map.Entry<VertexId, DoubleTensor> partial :
            ...
            if (partialIsLeft) {
                result =
                        partial.getValue()
                                .tensorMultiply(
                                        multiplier, new int[] {wrtRightDimension}, new int[] {1});
            } else {
                ...
                result =
                        partial.getValue()
                                .tensorMultiply(
                                        multiplier, new int[] {wrtLeftDimension}, new int[] {0})
    ...
    public PartialDerivatives divideBy(DoubleTensor divisor) {
        ...
        for (Map.Entry<VertexId, DoubleTensor> entry : derivativeWithRespectTo.entrySet()) {
            ...
            DoubleTensor v =
                    partial.div(increaseRankByAppendingOnesToShape(divisor, partial.getRank()));
            divided.put(k, v);
        }
    ...
    public PartialDerivatives reshape(int currentRank, int[] proposedShape) {
        ...
        for (Map.Entry<VertexId, DoubleTensor> partialDerivative :
                derivativeWithRespectTo.entrySet()) {
     ...
     * @param reshape Due to the way our tensor implementation works, slicing a rank 2 tensor gives
     *     us a rank two back, whereas slicing a higher rank tensor gives you a (rank - 1) tensor
     *     back. This causes problems for rank 2 tensors where the shape of the "of" will go from,
     *     say, 3x3 to 3x1 whereas the partial will go from 3x3x3x3 to 3x3x3 instead of 3x1x3x3.
     *     This reshape deals with this case. Only needed for rank two inputs as higher ranks
     *     correctly resolve (eg 3x3x3 will have a 3x3x3x3x3x3 and after slicing will be a 3x3 and a
     *     partial of 3x3x3x3x3.
    ...
    public PartialDerivatives slice(int dimension, int index, boolean reshape) {
        ...
        for (Map.Entry<VertexId, DoubleTensor> partialDerivative :
            ...
            DoubleTensor slicedPartialDerivative =
                    partialDerivative.getValue().slice(dimension, index);
    ...
    private int[] extractWrtShape(int[] partialDerivativeShape, int rankOfSource) {
        return Arrays.copyOfRange(
                partialDerivativeShape, rankOfSource, partialDerivativeShape.length);
    }
    ...
    private static DoubleTensor increaseRankByAppendingOnesToShape(
        ...
        return lowRankTensor.reshape(
                TensorShape.shapeDesiredToRankByAppendingOnes(
                        lowRankTensor.getShape(), desiredRank));
    }
