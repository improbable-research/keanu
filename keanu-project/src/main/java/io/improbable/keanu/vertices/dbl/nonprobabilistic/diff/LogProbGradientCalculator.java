...
public class LogProbGradientCalculator {
  ...
  public LogProbGradientCalculator(
      List<? extends Vertex> logProbOfVerticesList, List<? extends Vertex<?>> wrtVerticesList) {
   ...
   * each of its arguments and with respect to its value, x. This method searches these partials for
   * any that are parents of the vertices we are taking the derivative with respect to
   *
   * @param ofVertices the vertices that the derivative is being calculated "of" with respect to the
   *     wrtVertices
  ...
  private Map<Vertex, Set<DoubleVertex>> getVerticesWithNonzeroDiffWrt(
      Set<? extends Vertex<?>> ofVertices, Map<Vertex, Set<DoubleVertex>> parentToWrtVertices) {
    return ofVertices
        .stream()
        .collect(
            Collectors.toMap(
