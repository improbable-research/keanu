...
public abstract class Vertex<T> implements Observable<T>, Variable<T, VertexState<T>> {
    ...
    public String createDescriptionAllowingLabels() {
        Collection<Vertex> parents = this.getParents();
        ...
        return this.recursiveDescriptionStep(true);
    }

    protected String recursiveDescriptionStep(boolean includeBrackets) {
        String[] parentStrings = parents.stream().map(Vertex::createDescriptionAllowingLabels).toArray(String[]::new);

        CharSequence delimiter = this.getDescriptionDelimiter();
    ...
    public String createDescription() {
        ...
        if (parents.size() == 0) {
            ...
            return "Vertex with no parents and label: " + this.getLabel().toString();
        }

        String thisLabel = this.getLabel() != null ? this.getLabel().toString() : "This Vertex";

        return thisLabel + " = " + this.recursiveDescriptionStep(false);
    }
}
