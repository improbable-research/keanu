...
public abstract class Vertex<T> implements Observable<T> {
   ...
   * label name.
   *
   * @param label The label to apply to this vertex. Uniqueness is only enforced on instantiation of
   *     a Bayes Net
   */
  ...
  /**
   * This is similar to eval() except it only propagates as far up the graph as required until there
   * are values present to operate on. On a graph that is completely uninitialized, this would be
   * the same as eval()
   *
  ...
  /**
   * @return True if the vertex is probabilistic, false otherwise. A probabilistic vertex is defined
   *     as a vertex whose value is not derived from it's parents. However, the probability of the
