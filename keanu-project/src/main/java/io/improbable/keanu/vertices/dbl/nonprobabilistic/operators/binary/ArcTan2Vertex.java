...
public class ArcTan2Vertex extends DoubleBinaryOpVertex {
    ...
    /**
     * Calculates the signed angle, in radians, between the positive x-axis and a ray to the point
     * (x, y) from the origin
     *
        ...
        PartialDerivatives thisInfX =
                x.getPartialDerivatives()
                        .multiplyAlongOfDimensions(
                                (y.getValue().div(denominator)).unaryMinusInPlace(),
                                x.getValue().getShape());
        PartialDerivatives thisInfY =
                y.getPartialDerivatives()
                        .multiplyAlongOfDimensions(
                                x.getValue().div(denominator), y.getValue().getShape());
    ...
    public Map<Vertex, PartialDerivatives> reverseModeAutoDifferentiation(
        ...
        partials.put(
                ...
                derivativeOfOutputsWithRespectToSelf.multiplyAlongWrtDimensions(
                        dOutWrtX, this.getShape()));
        partials.put(
                right,
                derivativeOfOutputsWithRespectToSelf.multiplyAlongWrtDimensions(
                        dOutWrtY, this.getShape()));
        return partials;
    }
}
