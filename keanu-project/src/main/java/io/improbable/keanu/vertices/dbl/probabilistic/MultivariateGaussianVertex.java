...
public class MultivariateGaussianVertex extends DoubleVertex implements ProbabilisticDouble {
  ...
  /**
   * Multivariate gaussian distribution. The shape is driven from mu, which must be a vector. The
   * shape of the covariance (matrix) must be a square that is the same height as mu.
   *
  ...
  /**
   * Matches a mu to a Multivariate Gaussian. The covariance value provided here is used to create a
   * covariance tensor by multiplying the scalar value against an identity matrix of the appropriate
   * size.
   *
   * @param mu the mu of the Multivariate Gaussian
  ...
  public double logProb(DoubleTensor value) {
    ...
    return io.improbable.keanu.distributions.continuous.MultivariateGaussian.withParameters(
            muValues, covarianceValues)
        .logProb(value)
        .scalar();
  }
  ...
  public DoubleTensor sample(KeanuRandom random) {
    return MultivariateGaussian.withParameters(mu.getValue(), covariance.getValue())
        .sample(mu.getShape(), random);
  }
