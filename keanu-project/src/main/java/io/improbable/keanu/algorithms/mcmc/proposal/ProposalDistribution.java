...
public interface ProposalDistribution {
  ...
  /**
   * Represents q(x|x') where q is the proposal distribution, x' is the proposal to value and x is
   * the proposal from value.
   *
   * @param proposal A proposal value for each vertex that contains a from and a to value.
  ...
  default double logProbAtFromGivenTo(Proposal proposal) {
    ...
    for (Vertex v : proposal.getVerticesWithProposal()) {
      sumLogProb +=
          logProb((Probabilistic) v, proposal.getProposalFrom(v), proposal.getProposalTo(v));
    }
    return sumLogProb;
  }
  ...
  default double logProbAtToGivenFrom(Proposal proposal) {
    ...
    for (Vertex v : proposal.getVerticesWithProposal()) {
      sumLogProb +=
          logProb((Probabilistic) v, proposal.getProposalTo(v), proposal.getProposalFrom(v));
    }
    return sumLogProb;
  }
}
