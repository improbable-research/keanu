...
public class GaussianKDE {
  ...
  public static KDEVertex approximate(DoubleVertex vertex, Integer nSamples) {
    ...
    DoubleVertexSamples vertexSamples =
        MetropolisHastings.withDefaultConfig()
            .getPosteriorSamples(network, ImmutableList.of(vertex), nSamples)
            .getDoubleTensorSamples(vertex);
  ...
  private static DoubleTensor checkIfScalar(DoubleTensor tensor) throws IllegalArgumentException {
    if (!tensor.isScalar()) {
      throw new IllegalArgumentException(
          "The provided samples are not scalars, but have shape "
