...
/**
 * Metropolis Hastings is a Markov Chain Monte Carlo method for obtaining samples from a probability
 * distribution
 */
@Builder
public class MetropolisHastings implements PosteriorSamplingAlgorithm {
  ...
  public static MetropolisHastings withDefaultConfig(KeanuRandom random) {
    return MetropolisHastings.builder().random(random).build();
  }
  ...
  public NetworkSamples getPosteriorSamples(
      ...
      int sampleCount) {
    return generatePosteriorSamples(bayesianNetwork, verticesToSampleFrom).generate(sampleCount);
  }

  public NetworkSamples getPosteriorSamples(
      BayesianNetwork bayesianNetwork, Vertex vertexToSampleFrom, int sampleCount) {
    return generatePosteriorSamples(bayesianNetwork, Collections.singletonList(vertexToSampleFrom))
        .generate(sampleCount);
  }

  public NetworkSamplesGenerator generatePosteriorSamples(
      final BayesianNetwork bayesianNetwork, final List<? extends Vertex> verticesToSampleFrom) {

    return new NetworkSamplesGenerator(
        setupSampler(bayesianNetwork, verticesToSampleFrom), ProgressBar::new);
  }

  private SamplingAlgorithm setupSampler(
      final BayesianNetwork bayesianNetwork, final List<? extends Vertex> verticesToSampleFrom) {
    ...
    MetropolisHastingsStep mhStep =
        new MetropolisHastingsStep(
            latentVertices, proposalDistribution, useCacheOnRejection, random);

    double logProbabilityBeforeStep = bayesianNetwork.getLogOfMasterP();

    return new Sampler(
        latentVertices, verticesToSampleFrom, mhStep, variableSelector, logProbabilityBeforeStep);
  }
  ...
  public static class Sampler implements SamplingAlgorithm {
    ...
    public void step() {
      ...
      logProbabilityBeforeStep =
          mhStep.step(chosenVertices, logProbabilityBeforeStep).getLogProbabilityAfterStep();
    ...
    @Override
    public void sample(
        Map<VertexId, List<?>> samplesByVertex, List<Double> logOfMasterPForEachSample) {
      step();
  ...
  private static void takeSamples(
  ...
  private static <T> void addSampleForVertex(Vertex<T> vertex, Map<VertexId, List<?>> samples) {
    List<T> samplesForVertex =
        (List<T>) samples.computeIfAbsent(vertex.getId(), v -> new ArrayList<T>());
