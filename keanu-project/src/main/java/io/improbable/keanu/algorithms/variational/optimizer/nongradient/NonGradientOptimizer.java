...
public class NonGradientOptimizer implements Optimizer {
  ...
  @Getter private final BayesianNetwork bayesianNetwork;
  ...
  @Builder.Default private final double boundsRange = Double.POSITIVE_INFINITY;

  /** bounds for each specific continuous latent vertex */
  @Builder.Default private final OptimizerBounds optimizerBounds = new OptimizerBounds();

  /** radius around region to start testing points */
  @Builder.Default double initialTrustRegionRadius = BOBYQAOptimizer.DEFAULT_INITIAL_RADIUS;

  /** stopping trust region radius */
  @Builder.Default double stoppingTrustRegionRadius = BOBYQAOptimizer.DEFAULT_STOPPING_RADIUS;
  ...
  private double optimize(List<Vertex> outputVertices) {
    ...
    FitnessFunction fitnessFunction =
        new FitnessFunction(outputVertices, latentVertices, this::handleFitnessCalculation);
    ...
    ApacheMathSimpleBoundsCalculator boundsCalculator =
        new ApacheMathSimpleBoundsCalculator(boundsRange, optimizerBounds);
