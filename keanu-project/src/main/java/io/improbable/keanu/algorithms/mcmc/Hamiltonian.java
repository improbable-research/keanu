...
/**
 * Hamiltonian Monte Carlo is a method for obtaining samples from a probability distribution with
 * the introduction of a momentum variable.
 *
 * <p>Algorithm 1: "Hamiltonian Monte Carlo". The No-U-Turn Sampler: Adaptively Setting Path Lengths
 * in Hamiltonian Monte Carlo https://arxiv.org/pdf/1111.4246.pdf
 */
@Builder
public class Hamiltonian implements PosteriorSamplingAlgorithm {
    ...
    public static Hamiltonian withDefaultConfig(KeanuRandom random) {
        return Hamiltonian.builder().random(random).build();
    }
    ...
    public NetworkSamples getPosteriorSamples(
        ...
        for (int sampleNum = 1; sampleNum < sampleCount; sampleNum++) {
            ...
            final double likelihoodOfLeapfrog =
                    getLikelihoodOfLeapfrog(
    ...
    private static void initializeMomentumForEachVertex(
            List<Vertex<DoubleTensor>> vertexes,
    ...
    /**
     * function Leapfrog(T, r) Set `r = r + (eps/2)dTL(T) Set `T = T + r` Set `r = r` +
     * (eps/2)dTL(`T) return `T, r`
     *
     ...
     */
    private static Map<VertexId, DoubleTensor> leapfrog(
        ...
        for (Map.Entry<VertexId, DoubleTensor> vertexMomentum : momentums.entrySet()) {
            final DoubleTensor updatedMomentum =
                    gradient.get(vertexMomentum.getKey())
                            .times(halfTimeStep)
                            .plusInPlace(vertexMomentum.getValue());
            momentumsAtHalfTimeStep.put(vertexMomentum.getKey(), updatedMomentum);
        }

        // Set `T = T + `r.
        for (Vertex<DoubleTensor> latent : latentVertices) {
            final DoubleTensor nextPosition =
                    momentumsAtHalfTimeStep
                            .get(latent.getId())
                            .times(halfTimeStep)
        ...
        // Set `r = `r + (eps/2)dTL(`T)
        Map<VertexId, DoubleTensor> newGradient =
                logProbGradient.getJointLogProbGradientWrtLatents();

        for (Map.Entry<VertexId, DoubleTensor> halfTimeStepMomentum :
                momentumsAtHalfTimeStep.entrySet()) {
            final DoubleTensor updatedMomentum =
                    newGradient
                            .get(halfTimeStepMomentum.getKey())
                            .times(halfTimeStep)
                            .plusInPlace(halfTimeStepMomentum.getValue());
    ...
    private static double getLikelihoodOfLeapfrog(
            final double logOfMasterPAfterLeapfrog,
        ...
        final double leapFroggedLikelihood =
                logOfMasterPAfterLeapfrog - leapFroggedMomentumDotProduct;
    ...
    /**
     * This is meant to be used for caching a pre-leapfrog sample. This sample will be used if the
     * leapfrog is rejected.
     *
     * @param sample
     * @param fromVertices
     */
    ...
    /**
     * This is used when a leapfrog is rejected. At that point the vertices are in a post leapfrog
     * state and a pre-leapfrog sample must be used.
     *
     * @param samples
    ...
    /**
     * This is used when a leapfrog is accepted. At that point the vertices are in a post leapfrog
     * state.
     *
     * @param samples
     * @param fromVertices vertices from which to create and save new sample.
     */
    private static void addSampleFromVertices(
            Map<VertexId, List<?>> samples, List<? extends Vertex> fromVertices) {
        for (Vertex<?> vertex : fromVertices) {
