...
class MetropolisHastingsStep {
   ...
   * @param useCacheOnRejection True if caching values of the network such that recalculation isn't
   *     required on step rejection
  ...
  MetropolisHastingsStep(
    ...
    this.random = random;
    this.affectedVerticesCache = createVerticesAffectedByCache(latentVertices, useCacheOnRejection);
  }

  public StepResult step(final Set<Vertex> chosenVertices, final double logProbabilityBeforeStep) {
   ...
   * @param temperature Temperature for simulated annealing. This should be constant if no annealing
   *     is wanted
  ...
  private static Map<Vertex, LambdaSection> createVerticesAffectedByCache(
    ...
    return latentVertices
        ...
        .collect(
            Collectors.toMap(
                v -> v, v -> LambdaSection.getDownstreamLambdaSection(v, useCacheOnRejection)));
  }
