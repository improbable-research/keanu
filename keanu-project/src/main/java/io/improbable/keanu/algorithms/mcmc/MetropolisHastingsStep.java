...
class MetropolisHastingsStep {
     ...
     * @param useCacheOnRejection True if caching values of the network such that recalculation
     *     isn't required on step rejection
    ...
    MetropolisHastingsStep(
        ...
        this.random = random;
        this.affectedVerticesCache =
                createVerticesAffectedByCache(latentVertices, useCacheOnRejection);
    }

    public StepResult step(
            final Set<Vertex> chosenVertices, final double logProbabilityBeforeStep) {
     ...
     * @param temperature Temperature for simulated annealing. This should be constant if no
     *     annealing is wanted
     * @return the log probability of the network after either accepting or rejecting the sample
     */
    public StepResult step(
            final Set<Vertex> chosenVertices,
            final double logProbabilityBeforeStep,
            final double temperature) {

        final double affectedVerticesLogProbOld =
                sumLogProbabilityOfAffected(chosenVertices, affectedVerticesCache);

        NetworkSnapshot preProposalSnapshot = null;
        if (useCacheOnRejection) {
            preProposalSnapshot =
                    getSnapshotOfAllAffectedVertices(chosenVertices, affectedVerticesCache);
        }
        ...
        final double affectedVerticesLogProbNew =
                sumLogProbabilityOfAffected(chosenVertices, affectedVerticesCache);

        if (affectedVerticesLogProbNew != LOG_ZERO_PROBABILITY) {

            final double logProbabilityDelta =
                    affectedVerticesLogProbNew - affectedVerticesLogProbOld;
    ...
    private static NetworkSnapshot getSnapshotOfAllAffectedVertices(
            final Set<Vertex> chosenVertices, final Map<Vertex, LambdaSection> affectedVertices) {
    ...
    private static double sumLogProbabilityOfAffected(
            Set<Vertex> vertices, Map<Vertex, LambdaSection> affectedVertices) {
        double sumLogProb = 0.0;
        for (Vertex v : vertices) {
            sumLogProb +=
                    ProbabilityCalculator.calculateLogProbFor(
                            affectedVertices.get(v).getLatentAndObservedVertices());
        }
        return sumLogProb;
    }

    /**
     * This creates a cache of potentially all vertices downstream to an observed or probabilistic
     * vertex from each latent vertex. If useCacheOnRejection is false then only the downstream
     * observed or probabilistic is cached.
     *
     * @param latentVertices The latent vertices to create a cache for
     * @param useCacheOnRejection Whether or not to cache the entire downstream set or just the
     *     observed/probabilistic
     * @return A vertex to Lambda Section map that represents the downstream Lambda Section for each
     *     latent vertex. This Lambda Section may include all of the nonprobabilistic vertices if
     *     useCacheOnRejection is enabled.
     */
    private static Map<Vertex, LambdaSection> createVerticesAffectedByCache(
            List<? extends Vertex> latentVertices, boolean useCacheOnRejection) {
        return latentVertices
                ...
                .collect(
                        Collectors.toMap(
                                v -> v,
                                v ->
                                        LambdaSection.getDownstreamLambdaSection(
                                                v, useCacheOnRejection)));
    }
