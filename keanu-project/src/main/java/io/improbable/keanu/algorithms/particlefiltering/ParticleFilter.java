...
public class ParticleFilter {
   ...
   * latent variables at each stage. Each increment involves the addition of one new observed vertex
   * and the latent vertices that it depend on. This is done for a specified number of 'particles',
   * each of which represents one set of randomly sampled values of the latent vertices in the
   * Bayesian network and has an associated probability. As the proportion of the graph under
   * consideration incrementally grows, less probable particles are culled and more probable
   * particles are duplicated, resulting in a final set of relatively probable particles.
   *
   * <p>This methodology is similar to the Sequential Importance Resampling Algorithm described here
   ...
   *     probability particles are replicated each time the subgraph under consideration is expanded
   * @param resamplingProportion the proportion of particles to cull (e.g. the 25% of least probably
   *     particles could be culled)
