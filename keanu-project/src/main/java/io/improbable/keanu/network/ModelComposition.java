...
public final class ModelComposition {
     ...
     *
     * <p>This function will take a previously constructed BayesNet and do a number of things: -
     * Hook up any Proxy Vertices to the parents specified in the inputVertices - Increment the
     * depth for all "internal" nodes and the BayesNet itself - Pass back any specified output nodes
     * from the Bayesnet (keeping them at the outer depth)
     *
     * <p>Output nodes will be returned unlabelled
     *
     ...
     */
    public static Map<VertexLabel, Vertex> composeModel(
    ...
    private static Map<VertexLabel, Vertex> extractOutputs(
            BayesianNetwork bayesianNetwork, List<VertexLabel> desiredOutputs) {
    ...
    private static void increaseDepth(
            BayesianNetwork bayesianNetwork, Map<VertexLabel, Vertex> outputVertices) {
        ...
        for (Map.Entry<VertexLabel, Vertex> entry : inputs.entrySet()) {
            ...
            if (v instanceof ProxyVertex) {
                ...
                if (proxyVertex.hasParent()) {
                    throw new IllegalArgumentException(
                            "Proxy Vertex for \"" + v.getLabel() + "\" already has Parent");
                } else {
                    proxyVertex.setParent(entry.getValue());
                }
            } else {
                throw new IllegalArgumentException(
                        "Input node \"" + entry.getKey() + "\" is not a Proxy Vertex");
            }
        }
    }

    private static void cleanOutputLabels(Map<VertexLabel, Vertex> outputMap) {
        outputMap.values().stream().forEach(v -> v.setLabel(null));
    }
}
