...
public class Multinomial implements DiscreteDistribution {
   ...
   * @param p The probability of observing each of the k values (which sum to 1) p is a Tensor whose
   *     first dimension must be of size k
   * @see <a href="https://en.wikipedia.org/wiki/Multinomial_distribution">Multinomial
   *     Distribution</a> Generalisation of the Binomial distribution to variables with more than 2
   *     possible values
   */
  ...
  public IntegerTensor sample(int[] shape, KeanuRandom random) {
    ...
    for (int i = 0; i < length; i++) {
      ...
      int[] sample =
          drawNTimes(
              nFlattened.getOrScalar(i), random, categoryProbabilities.toArray(new Double[0]));
  ...
  /**
   * This method is necessary because I've constructed a flat array by concatenation samples of size
   * k So for example, if the shape of n is [a, b] then I've now got a tensor of shape [a, b, k]
   * which I need to convert to a tensor of shape [k, a, b] by doing a slice in the highest
  ...
  public DoubleTensor logProb(IntegerTensor k) {
    ...
    Preconditions.checkArgument(
        k.greaterThanOrEqual(0).allTrue(), String.format("Inputs %s cannot be negative", k));
