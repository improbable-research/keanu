...
/**
 * The Smooth Uniform distribution is the usual Uniform distribution with the edges at max and min
 * smoothed by attaching a sigmoid as shoulders.
 *
 * <p>Math:
 *
 * <p>let sigmoid shoulder function be f(x), Sw be shoulder width, Bw be base (max-min) width, and h
 * be the bodyHeight of the base.
 *
 * <p>f(x) = Ax^3 + Bx^2 f'(x) = 3Ax^2 + 2Bx integral f = Ax^4/4 + Bx^3/3
 *
 * <p>f(Sw) = h integral of f from 0 to Sw = 1 (area under the curve must be 1) f'(Sw) = 0
 *
 * <p>yields:
 *
 * <p>| 0 3Sw^2 2Sw | | h | | 0 | | -1 Sw^3 Sw^2 | * | A | = | 0 | | Bw Sw/4 2Sw/3 | | B | | 1 |
 *
 * <p>therefore:
 *
 * <p>h = 1 / (Sw + Bw) A = -2 / (Sw^3 * (Sw + Bw)) B = 3 / (Sw^3 * Sw^2*Bw)
 */
public class SmoothUniform implements ContinuousDistribution {
    ...
    /**
     * Will return samples between xMin and xMax as well as samples from the left and right
     * shoulder. The width of the shoulder is determined by the edgeSharpness as a percentage of the
     * body width, which is (xMax - xMin).
     *
    ...
    public static ContinuousDistribution withParameters(
            DoubleTensor xMin, DoubleTensor xMax, double edgeSharpness) {
    ...
    public DoubleTensor sample(int[] shape, KeanuRandom random) {
        ...
        final DoubleTensor rScaled =
                r1.timesInPlace(bodyWidth.plus(shoulderWidth))
                        .plusInPlace(xMin.minus(shoulderWidth.div(2)));
        ...
        final DoubleTensor inverseFirstConditional =
                DoubleTensor.ones(firstConditional.getShape()).minusInPlace(firstConditional);
        ...
        final DoubleTensor secondConditionalNestedTrue =
                secondConditional.times(r2.getLessThanOrEqualToMask(acceptProbability));
        final DoubleTensor secondConditionalNestedFalse =
                secondConditional.timesInPlace(r2.getGreaterThanMask(acceptProbability));
        final DoubleTensor secondConditionalNestedFalseResult =
                xMin.minus(shoulderWidth).plusInPlace(spillOnToShoulder);
        ...
        final DoubleTensor secondConditionalFalseNestedTrue =
                secondConditionalFalse.times(r2.getLessThanOrEqualToMask(acceptProbability));
        final DoubleTensor secondConditionalFalseNestedFalse =
                secondConditionalFalse.timesInPlace(r2.getGreaterThanMask(acceptProbability));
        final DoubleTensor secondConditionalFalseNestedFalseResult =
                shoulderWidth.plusInPlace(xMax).minusInPlace(spillOnToShoulder);

        return firstConditional
                .timesInPlace(rScaled)
                .plusInPlace(
                        inverseFirstConditional
                                .times(secondConditionalNestedTrue)
                                .timesInPlace(rScaled))
                .plusInPlace(
                        inverseFirstConditional
                                .times(secondConditionalNestedFalse)
                                .timesInPlace(secondConditionalNestedFalseResult))
                .plusInPlace(
                        inverseFirstConditional
                                .times(secondConditionalFalseNestedTrue)
                                .timesInPlace(rScaled))
                .plusInPlace(
                        inverseFirstConditional
                                .timesInPlace(secondConditionalFalseNestedFalse)
                                .timesInPlace(secondConditionalFalseNestedFalseResult));
    }
    ...
    public DoubleTensor logProb(DoubleTensor x) {
        ...
        final DoubleTensor secondConditionalResult =
                shoulder(shoulderWidth, bodyWidth, x.minus(leftCutoff));
        ...
        final DoubleTensor thirdConditionalResult =
                shoulder(shoulderWidth, bodyWidth, shoulderWidth.minus(x).plusInPlace(xMax));

        return firstConditional
                .timesInPlace(firstConditionalResult)
    ...
    public Diffs dLogProb(DoubleTensor x) {
        ...
        final DoubleTensor firstConditionalResult =
                dShoulder(shoulderWidth, bodyWidth, x.minus(leftCutoff));
        ...
        final DoubleTensor secondConditionalResult =
                dShoulder(shoulderWidth, bodyWidth, shoulderWidth.minus(x).plusInPlace(rightCutoff))
                        .unaryMinusInPlace();
        ...
        return new Diffs()
                .put(
                        ...
                        firstConditional
                                .timesInPlace(firstConditionalResult)
                                .plusInPlace(
                                        secondConditional.timesInPlace(secondConditionalResult)));
    }
    ...
    private static DoubleTensor dShoulder(DoubleTensor Sw, DoubleTensor Bw, DoubleTensor x) {
        final DoubleTensor A = getCubeCoefficient(Sw, Bw);
        final DoubleTensor B = getSquareCoefficient(Sw, Bw);
        return A.timesInPlace(3)
                .timesInPlace(x.pow(2))
                .plusInPlace(B.timesInPlace(x).timesInPlace(2));
    }
