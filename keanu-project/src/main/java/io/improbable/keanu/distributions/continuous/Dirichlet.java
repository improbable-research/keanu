...
public class Dirichlet implements ContinuousDistribution {
  ...
  public DoubleTensor sample(int[] shape, KeanuRandom random) {
    final ContinuousDistribution gamma =
        Gamma.withParameters(DoubleTensor.ones(shape), concentration);
  ...
  public DoubleTensor logProb(DoubleTensor x) {
    if (Math.abs(x.sum() - 1.0) > EPSILON) {
      throw new IllegalArgumentException(
          "Sum of values to calculate Dirichlet likelihood for must equal 1");
    }
    ...
    final double logGammaSumConcentration =
        org.apache.commons.math3.special.Gamma.logGamma(concentration.sum());
    return DoubleTensor.scalar(
        sumConcentrationLogged - sumLogGammaConcentration + logGammaSumConcentration);
  }

  @Override
  public Diffs dLogProb(DoubleTensor x) {
    final DoubleTensor dLogPdc =
        x.log()
    ...
    return new Diffs().put(C, dLogPdc).put(X, dLogPdx);
  }
