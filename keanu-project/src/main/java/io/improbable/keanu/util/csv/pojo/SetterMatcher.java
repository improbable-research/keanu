...
/**
 * This finds an appropriate setter method for a given csv title. This is done by case insensitive
 * name or by annotation.
 */
public class SetterMatcher {

    private SetterMatcher() {}

    public static <T> Optional<CsvCellConsumer<T>> getSetterCellConsumer(
            String title, List<Method> potentialMethods) {

        final Optional<Method> matchingMethodMaybe =
                findMatchingSetter(title.trim(), potentialMethods);

        return matchingMethodMaybe.map(SetterMatcher::createCellConsumerForMethod);
    }
    ...
    private static <T> CsvCellConsumer<T> createCellConsumerForMethod(Method method) {
        ...
        return (target, value) -> {
            Object convertedValue =
                    CsvCellDeserializer.convertToAppropriateType(
    ...
    public static <T> Optional<CsvColumnConsumer<T>> getSetterColumnConsumer(
            String title, List<Method> potentialMethods) {

        final Optional<Method> matchingMethodMaybe =
                findMatchingSetter(title.trim(), potentialMethods);

        return matchingMethodMaybe.map(SetterMatcher::createColumnConsumerForMethod);
    }

    private static <T> CsvColumnConsumer<T> createColumnConsumerForMethod(Method method) {
        return (target, value) -> {
            Object convertedValue =
                    CsvColumnDeserializer.convertToAppropriateType(
                            value, method.getParameterTypes()[0]);
     ...
     * @return A method that either matches by name prepended with set (e.g. myValue would match
     *     setMyValue) or has a CsvProperty annotation with an EXACT name match.
     */
    private static Optional<Method> findMatchingSetter(
            final String title, final List<Method> potentials) {
        return potentials
                .stream()
                .filter(SetterMatcher::methodTakesOnlyOneArgument)
                .filter(
                        method ->
                                titleMatchesSetterMethod(method, title)
                                        || methodContainsPropertyAnnotation(method, title))
                .findFirst();
    }
