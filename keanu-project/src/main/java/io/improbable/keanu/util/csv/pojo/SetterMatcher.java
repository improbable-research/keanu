...
public class SetterMatcher {
  ...
  private static <T> CsvCellConsumer<T> createCellConsumerForMethod(Method method) {
    ...
    return (target, value) -> {
      Object convertedValue =
          CsvCellDeserializer.convertToAppropriateType(value, method.getParameterTypes()[0]);
  ...
  private static <T> CsvColumnConsumer<T> createColumnConsumerForMethod(Method method) {
    return (target, value) -> {
      Object convertedValue =
          CsvColumnDeserializer.convertToAppropriateType(value, method.getParameterTypes()[0]);
