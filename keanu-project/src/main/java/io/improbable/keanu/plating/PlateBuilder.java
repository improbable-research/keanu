...
public class PlateBuilder<T> {
  ...
  private interface PlateFactory {
     ...
     * @throws PlateConstructionException which can occur e.g. if the labels don't marry up in the
     *     transition mapping
     */
    Plates build();
  }
  ...
  public class FromDataFactory implements PlateFactory {
    ...
    private FromDataFactory(
        BiConsumer<Plate, T> factory, PlateData<T> data, int size, VertexDictionary initialState) {
