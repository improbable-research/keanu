...
public class LoopBuilder {
  ...
  /**
   * An optional method to override the default behaviour If the maximum loop count is exceeded, it
   * will log a warning instead of throwing
   *
   * @return self
   */
   ...
   * @param proxysParentLabel the label assigned to a Vertex in frame t-1 which will become the
   *     ProxyVertex's parent
   * @return self
   */
   ...
   * A mandatory method to specify the condition
   *
   * @param conditionFunction a lambda that takes the current Plate and creates and returns a new
   *     BoolVertex
   * @return the next stage builder
   */
  public LoopBuilder2 iterateWhile(Function<Plate, BoolVertex> conditionFunction) {
    return new LoopBuilder2(
        initialState,
        conditionFunction,
        customMappings.build(),
        maxLoopCount,
        throwWhenMaxCountIsReached);
  }
  ...
  public class LoopBuilder2 {
    ...
    private final VertexLabel VALUE_OUT_WHEN_ALWAYS_TRUE_LABEL =
        new VertexLabel("loop_value_out_when_always_true");
    private final VertexLabel VALUE_IN_WHEN_ALWAYS_TRUE_LABEL =
        PlateBuilder.proxyFor(VALUE_OUT_WHEN_ALWAYS_TRUE_LABEL);
    private final VertexLabel LOOP_LABEL = new VertexLabel("loop");

    LoopBuilder2(
        VertexDictionary initialState,
        Function<Plate, BoolVertex> conditionFunction,
        Map<VertexLabel, VertexLabel> customMappings,
        int maxLoopCount,
        boolean throwWhenMaxCountIsReached) {
    ...
    private VertexDictionary setInitialState(VertexDictionary initialState) {
      ...
      try {
        ...
        valueOutWhenAlwaysTrue =
            new DoubleProxyVertex(
                VALUE_OUT_WHEN_ALWAYS_TRUE_LABEL.withExtraNamespace("Loop_" + this.hashCode()));
        valueOutWhenAlwaysTrue.setParents(outputVertex);
      } catch (NoSuchElementException e) {
        throw new LoopConstructionException(
            "You must pass in a base case, i.e. a vertex labeled as Loop.VALUE_OUT_LABEL", e);
      } catch (IllegalArgumentException e) {
        throw new LoopConstructionException(
            "You must pass in only one vertex labeled as Loop.VALUE_OUT_LABEL", e);
      }

      return initialState.withExtraEntries(
          ImmutableMap.of(
              VALUE_OUT_WHEN_ALWAYS_TRUE_LABEL,
              valueOutWhenAlwaysTrue,
              LOOP_LABEL,
              ConstantVertex.of(true)));
    }

    /**
     * A mandatory method to specify the iteration step
     *
     * @param iterationFunction a lambda that takes the Proxy input vertex and creates and returns a
     *     new output Vertex
     ...
     * A mandatory method to specify the iteration step
     *
     * @param iterationFunction a lambda that takes the current Plate and the Proxy input vertex and
     *     creates and returns a new output vertex
     * @return the fully constructed Loop object
     */
      ...
      Plates plates =
          new PlateBuilder<Integer>()
              ...
              .withFactory(
                  (plate) -> {
                    // inputs
                    DoubleVertex valueInWhenAlwaysTrue =
                        new DoubleProxyVertex(VALUE_IN_WHEN_ALWAYS_TRUE_LABEL);
                    ...
                    // outputs
                    DoubleVertex iterationResult =
                        iterationFunction.apply(plate, valueInWhenAlwaysTrue);
                    BoolVertex loopAgain = stillLooping.and(condition);
                    DoubleVertex result =
                        If.isTrue(loopAgain).then(iterationResult).orElse(valueIn);
