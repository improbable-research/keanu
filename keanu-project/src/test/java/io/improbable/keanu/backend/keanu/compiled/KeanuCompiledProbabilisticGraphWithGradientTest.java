...
public class KeanuCompiledProbabilisticGraphWithGradientTest {
    ...
    public void canAutoDiffTensorConcat() {
        ...
        try (ProbabilisticModelWithGradient graph = KeanuCompiledProbabilisticGraphWithGradient.convert(network)) {
    ...
    public void canRunLogProbabilityAndGradientLogProbabilityOfGaussian() {
        ...
        try (ProbabilisticModelWithGradient graph = KeanuCompiledProbabilisticGraphWithGradient.convert(network)) {
            ...
            Map<? extends VariableReference, DoubleTensor> gradients = graph.logProbGradients(inputs);

            assertEquals(keanuGradients.get(A.getId()), gradients.get(A.getReference()));
            assertEquals(keanuGradients.get(B.getId()), gradients.get(B.getReference()));
