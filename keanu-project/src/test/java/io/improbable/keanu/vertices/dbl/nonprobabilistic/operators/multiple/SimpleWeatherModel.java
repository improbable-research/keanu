...
public class SimpleWeatherModel {
  ...
  public SimpleWeatherModel(DoubleVertex inputToModel) throws IOException {
    ...
    when(humidityReader.readLine())
        .thenAnswer(
            invocation ->
                String.valueOf(blackBoxHumidityModel(this.inputToModel.getValue().scalar())));
    ...
    when(suggestedFactorSuncreamReader.readLine())
        .thenAnswer(
            invocation ->
                String.valueOf(blackBoxSunCreamModel(this.inputToModel.getValue().scalar())));

    isSunnyReader = mock(BufferedReader.class);
    when(isSunnyReader.readLine())
        .thenAnswer(
            invocation ->
                String.valueOf(blackBoxIsSunnyModel(this.inputToModel.getValue().scalar())));
  }
  ...
  public void modelExecution(Map<VertexLabel, Vertex<? extends Tensor>> inputs) {
    ...
    try {
      ...
      FileUtils.writeStringToFile(
          File.createTempFile("chanceOfRainResults", "csv"), String.valueOf(chanceOfRain));
      FileUtils.writeStringToFile(
  ...
  public Map<VertexLabel, Tensor> updateValuesMultipleTypes(
    ...
    try {
      ...
      modelOutput.put(
          new VertexLabel("suggestedFactorSuncream"), IntegerTensor.scalar(chanceOfRainResult));
