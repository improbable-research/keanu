...
public class Nd4jDoubleTensorTest {
    ...
    public void canMultiplyMatrixByScalar() {
        ...
        assertArrayEquals(new double[]{2, 4, 6, 8}, result.getLinearView(), 0.0);
    }
    ...
    public void canElementWiseDivideMatrix() {
        ...
        assertArrayEquals(new double[]{1.0, 1.0, 1.0, 1.0}, result.getLinearView(), 0.0);
    }

    @Test
    public void canDivideMatrixByScalar() {
        DoubleTensor result = matrixA.div(scalarA);
        assertArrayEquals(new double[]{1.0 / 2.0, 2.0 / 2.0, 3.0 / 2.0, 4.0 / 2.0}, result.getLinearView(), 0.0);
    }

    @Test
    public void canDivideScalarByMatrix() {
        DoubleTensor result = scalarA.div(matrixA);
        assertArrayEquals(new double[]{2.0 / 1.0, 2.0 / 2.0, 2.0 / 3.0, 2.0 / 4.0}, result.getLinearView(), 0.0);
    }
