...
public class CategoricalVertexTest {
  ...
  private <T> Map<T, Double> testSample(Map<T, DoubleVertex> selectableValues, KeanuRandom random) {
  ...
  private <T> void assertProportionsWithinExpectedRanges(
    ...
    for (Map.Entry<T, Double> entry : proportions.entrySet()) {
      ...
      assertEquals(
          String.format("Sample proportion for category %s is not as expected", entry.getKey()),
          p,
          expected,
          epsilon);
    }
