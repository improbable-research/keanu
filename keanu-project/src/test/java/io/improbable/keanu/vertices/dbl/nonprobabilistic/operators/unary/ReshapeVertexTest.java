...
public class ReshapeVertexTest {
    ...
    public void reshapeVertex() {
        ...
        Assert.assertArrayEquals(
                new double[] {1, 2, 3, 4}, reshapeVertex.getValue().asFlatDoubleArray(), 1e-6);
    }
    ...
    public void reshapeCorrectlyReshapesPartialDerivative() {
        ...
        PartialDerivatives backward =
                Differentiator.reverseModeAutoDiff(reshapedN, ImmutableSet.of(m, alpha));
        ...
        double[] expectedPartial =
                N.getDualNumber().getPartialDerivatives().withRespectTo(m).asFlatDoubleArray();

        Assert.assertArrayEquals(
                expectedPartial, forward.withRespectTo(m).asFlatDoubleArray(), 1e-6);
        Assert.assertArrayEquals(
                expectedPartial, backward.withRespectTo(m).asFlatDoubleArray(), 1e-6);
    }
    ...
    public void flatPartialDerivativeIsTheSameAfterReshape() {
        ...
        ReshapeVertex reshapedN = new ReshapeVertex(N, 4, 1);
        DoubleTensor reshapedPartialWrtM =
                reshapedN.getDualNumber().getPartialDerivatives().withRespectTo(m);
        DoubleTensor reshapedPartialWrtA =
                reshapedN.getDualNumber().getPartialDerivatives().withRespectTo(a);

        Assert.assertArrayEquals(
                nWrtMpartialsBeforeReshape, reshapedPartialWrtM.asFlatDoubleArray(), 1e-6);
        Assert.assertArrayEquals(
                nWrtApartialsBeforeReshape, reshapedPartialWrtA.asFlatDoubleArray(), 1e-6);
    }
    ...
    public void partialCorrectlyFlowsThroughReshape() {
        ...
        Assert.assertArrayEquals(
                forward.withRespectTo(A).asFlatDoubleArray(),
    ...
    public void changesMatchGradient() {
        ...
        finiteDifferenceMatchesGradient(
                ImmutableList.of(inputVertex), outputVertex, 10.0, 1e-10, true);
    }
}
