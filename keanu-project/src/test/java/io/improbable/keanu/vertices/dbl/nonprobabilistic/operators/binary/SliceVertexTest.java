...
public class SliceVertexTest {
    ...
    public void setup() {
        matrixA =
                new ConstantDoubleVertex(
                        DoubleTensor.create(new double[] {1, 2, 3, 4, 5, 6}, 2, 3));
    }
    ...
    public void sliceCorrectlySplitsRowOfPartialDerivativeDimZeroIndexZero() {
        assertPartialsOfSliceWithRespectToOriginalAreCorrect(
                0, 0, new double[] {140, 170}, new int[] {1, 2}, new int[] {1, 2, 2, 3});
    }

    @Test
    public void sliceCorrectlySplitsRowOfPartialDerivativeDimZeroIndexOne() {
        assertPartialsOfSliceWithRespectToOriginalAreCorrect(
                0, 1, new double[] {320, 395}, new int[] {1, 2}, new int[] {1, 2, 2, 3});
    }

    @Test
    public void sliceCorrectlySplitsRowOfPartialDerivativeDimOneIndexZero() {
        assertPartialsOfSliceWithRespectToOriginalAreCorrect(
                1, 0, new double[] {140, 320}, new int[] {2, 1}, new int[] {2, 1, 2, 3});
    }

    @Test
    public void sliceCorrectlySplitsRowOfPartialDerivativeDimOneIndexOne() {
        assertPartialsOfSliceWithRespectToOriginalAreCorrect(
                1, 1, new double[] {170, 395}, new int[] {2, 1}, new int[] {2, 1, 2, 3});
    }

    private void assertPartialsOfSliceWithRespectToOriginalAreCorrect(
            int dim,
            int ind,
            double[] expectedValue,
            int[] expectedShape,
            int[] expectedPartialShape) {
        DoubleVertex m = new UniformVertex(0, 10);
        ...
        PartialDerivatives backward =
                Differentiator.reverseModeAutoDiff(sliceN, ImmutableSet.of(m, alpha));
        ...
        Assert.assertArrayEquals(
                originalPartial.slice(dim, ind).asFlatDoubleArray(),
                forward.withRespectTo(m).asFlatDoubleArray(),
                1e-6);
        Assert.assertArrayEquals(expectedPartialShape, forward.withRespectTo(m).getShape());

        Assert.assertArrayEquals(
                originalPartial.slice(dim, ind).asFlatDoubleArray(),
                backward.withRespectTo(m).asFlatDoubleArray(),
                1e-6);
    ...
    public void sliceCorrectlySplitsColumnOfPartialDerivative() {
        ...
        Assert.assertArrayEquals(
                sliceN.getValue().asFlatDoubleArray(), new double[] {65, 145}, 1e-6);
        Assert.assertArrayEquals(new int[] {2, 1}, sliceN.getShape());

        Assert.assertArrayEquals(
                originalPartial.slice(1, 1).asFlatDoubleArray(),
                slicePartial.asFlatDoubleArray(),
                1e-6);
