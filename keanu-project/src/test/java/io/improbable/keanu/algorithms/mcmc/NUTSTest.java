...
public class NUTSTest {
    ...
    public void samplesGaussian() {
        ...
        BayesianNetwork simpleGaussian =
                MCMCTestDistributions.createSimpleGaussian(mu, sigma, random);

        NUTS nuts = NUTS.builder().adaptCount(50).random(random).build();

        NetworkSamples posteriorSamples =
                nuts.getPosteriorSamples(simpleGaussian, simpleGaussian.getLatentVertices(), 1000);
    ...
    public void samplesContinuousPrior() {
        ...
        BayesianNetwork bayesNet =
                MCMCTestDistributions.createSumOfGaussianDistribution(20.0, 1.0, 46.);

        NUTS nuts = NUTS.builder().adaptCount(100).random(random).build();

        NetworkSamples posteriorSamples =
                nuts.getPosteriorSamples(bayesNet, bayesNet.getLatentVertices(), 2000);
    ...
    public void samplesFromDonut() {
        ...
        NetworkSamples samples =
                nuts.getPosteriorSamples(donutBayesNet, donutBayesNet.getLatentVertices(), 1000);
    ...
    public void canDefaultToSettingsInBuilderAndIsConfigurableAfterBuilding() {
        ...
        NUTS nuts = NUTS.builder().build();

        assertTrue(nuts.getAdaptCount() > 0);
        assertTrue(nuts.getTargetAcceptanceProb() > 0);
        assertNotNull(nuts.getRandom());

        NetworkSamples posteriorSamples = nuts.getPosteriorSamples(net, net.getLatentVertices(), 2);
