...
public class MCMCTestDistributions {
    ...
    public static BayesianNetwork createSimpleGaussian(
            double mu, double sigma, KeanuRandom random) {
    ...
    public static void samplesMatchSimpleGaussian(
            double mu, double sigma, List<DoubleTensor> samples) {

        int[] shape = samples.get(0).getShape();

        DoubleTensor summed =
                samples.stream().reduce(DoubleTensor.zeros(shape), DoubleTensor::plusInPlace);

        DoubleTensor averages = summed.divInPlace(samples.size());

        DoubleTensor sumDiffSquared =
                samples.stream()
                        .reduce(
                                DoubleTensor.zeros(shape),
                                (acc, tensor) ->
                                        acc.plusInPlace(tensor.minus(averages).powInPlace(2)));

        double[] standardDeviations =
                sumDiffSquared.div(samples.size() - 1).pow(0.5).asFlatDoubleArray();
    ...
    public static BayesianNetwork createSumOfGaussianDistribution(
            double mu, double sigma, double observedSum) {
    ...
    public static void samplesMatchesSumOfGaussians(
            double expected, List<DoubleTensor> sampleA, List<DoubleTensor> samplesB) {

        OptionalDouble averagePosteriorA =
                sampleA.stream()
                        .flatMapToDouble(tensor -> Arrays.stream(tensor.asFlatDoubleArray()))
                        .average();

        OptionalDouble averagePosteriorB =
                samplesB.stream()
                        .flatMapToDouble(tensor -> Arrays.stream(tensor.asFlatDoubleArray()))
                        .average();

        assertEquals(
                expected, averagePosteriorA.getAsDouble() + averagePosteriorB.getAsDouble(), 0.1);
    }
    ...
    public static void samplesMatch2DDonut(
            List<DoubleTensor> samplesA, List<DoubleTensor> samplesB) {
        ...
        for (int i = 0; i < samplesA.size(); i++) {
            ...
            if (sampleFromA > -0.2 && sampleFromA < 0.2 && sampleFromB > 0.6 && sampleFromB < 0.8) {
                topOfDonut = true;
            } else if (sampleFromA > 0.6
                    && sampleFromA < 0.8
                    && sampleFromB > -0.2
                    && sampleFromB < 0.2) {
                rightOfDonut = true;
            } else if (sampleFromA > -0.2
                    && sampleFromA < 0.2
                    && sampleFromB > -0.8
                    && sampleFromB < -0.6) {
                bottomOfDonut = true;
            } else if (sampleFromA > -0.8
                    && sampleFromA < -0.6
                    && sampleFromB > -0.2
                    && sampleFromB < 0.2) {
                leftOfDonut = true;
            } else if (sampleFromA > 0.35
                    && sampleFromA < 0.45
                    && sampleFromB > 0.35
                    && sampleFromB < 0.45) {
                middleOfDonut = true;
