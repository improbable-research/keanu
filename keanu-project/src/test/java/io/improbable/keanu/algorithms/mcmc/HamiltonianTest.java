...
public class HamiltonianTest {
  ...
  public void samplesGaussian() {
    double mu = 0.0;
    double sigma = 1.0;
    BayesianNetwork simpleGaussian =
        MCMCTestDistributions.createSimpleGaussian(mu, sigma, KeanuRandom.getDefaultRandom());

    Hamiltonian hmc = Hamiltonian.builder().leapFrogCount(10).stepSize(0.4).build();

    NetworkSamples posteriorSamples =
        hmc.getPosteriorSamples(simpleGaussian, simpleGaussian.getLatentVertices(), 1000);

    Vertex<DoubleTensor> vertex = simpleGaussian.getContinuousLatentVertices().get(0);

    MCMCTestDistributions.samplesMatchSimpleGaussian(
        mu, sigma, posteriorSamples.get(vertex).asList());
  }
  ...
  public void samplesContinuousPrior() {
    ...
    BayesianNetwork bayesNet =
        MCMCTestDistributions.createSumOfGaussianDistribution(20.0, 1.0, 46.0);

    Hamiltonian hmc = Hamiltonian.builder().leapFrogCount(20).stepSize(0.1).build();

    NetworkSamples posteriorSamples =
        hmc.getPosteriorSamples(bayesNet, bayesNet.getLatentVertices(), 2000);
    ...
    MCMCTestDistributions.samplesMatchesSumOfGaussians(
        44.0, posteriorSamples.get(A).asList(), posteriorSamples.get(B).asList());
  }
  ...
  public void samplesFromDonut() {
    ...
    Hamiltonian hmc = Hamiltonian.builder().leapFrogCount(15).stepSize(0.02).build();

    NetworkSamples samples =
        hmc.getPosteriorSamples(donutBayesNet, donutBayesNet.getLatentVertices(), 2500);
  ...
  public void canDefaultToSettingsInBuilderAndIsConfigurableAfterBuilding() {
    ...
    Hamiltonian hmc = Hamiltonian.builder().build();

    assertTrue(hmc.getLeapFrogCount() > 0);
    assertTrue(hmc.getStepSize() > 0);
    assertNotNull(hmc.getRandom());

    NetworkSamples posteriorSamples = hmc.getPosteriorSamples(net, net.getLatentVertices(), 2);
