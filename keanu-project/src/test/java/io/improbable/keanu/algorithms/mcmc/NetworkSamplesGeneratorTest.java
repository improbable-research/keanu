...
public class NetworkSamplesGeneratorTest {
    ...
    public void dropsAndSamplesExpectedNumberOfStepsOnGeneration() {
        ...
        NetworkSamplesGenerator unitUnderTest =
                new NetworkSamplesGenerator(algorithm, ProgressBar::new);
        ...
        int expectedCollected =
                (int) Math.ceil((totalGenerated - dropCount) / (double) downSampleInterval);
    ...
    public void streamsExpectedNumberOfSamples() {
        ...
        int downSampleInterval = 2;
        unitUnderTest.dropCount(dropCount).downSampleInterval(downSampleInterval);
        unitUnderTest.stream().limit(totalCollected).collect(Collectors.toList());
    ...
    public void doesUpdateProgressAndFinishProgressOnGeneration() {
        ...
        NetworkSamplesGenerator unitUnderTest =
                new NetworkSamplesGenerator(algorithm, () -> progressBar);
    ...
    public void doesUpdateProgressAndFinishProgressWhenStreaming() {
        ProgressBar progressBar = mock(ProgressBar.class);
        TestSamplingAlgorithm algorithm =
                new TestSamplingAlgorithm(new AtomicInteger(0), new AtomicInteger(0));
        Stream<NetworkState> sampleStream =
                new NetworkSamplesGenerator(algorithm, () -> progressBar).stream();
