...
def If(predicate: Union[tensor_arg_types, Vertex], thn: Union[tensor_arg_types, Vertex],
    ...
    if then_type == float or else_type == float:
        return Double(context.jvm_view().WhereVertex, None, cast_to_boolean_vertex(predicate),
                      cast_to_double_vertex(thn), cast_to_double_vertex(els))
    elif then_type == int or else_type == int:
        return Integer(context.jvm_view().WhereVertex, None, cast_to_boolean_vertex(predicate),
                       cast_to_integer_vertex(thn), cast_to_integer_vertex(els))
    elif then_type == bool and else_type == bool:
        return Boolean(context.jvm_view().WhereVertex, None, cast_to_boolean_vertex(predicate),
