...
public class WetGrass {
  ...
  public static void main(String[] args) {
    ...
    BoolVertex sprinkler = new BernoulliVertex(If.isTrue(rain).then(0.01).orElse(0.4));
    ...
    Stream<NetworkState> networkSamplesStream =
        MetropolisHastings.withDefaultConfig()
            .generatePosteriorSamples(
                new BayesianNetwork(wetGrass.getConnectedGraph()), Arrays.asList(sprinkler, rain))
            .dropCount(10000)
            .downSampleInterval(2)
            .stream();

    double probabilityOfRainGivenWetGrass =
        networkSamplesStream
                .limit(keepSampleCount)
                .filter(sample -> sample.get(rain).scalar())
                .count()
            / (double) keepSampleCount;
